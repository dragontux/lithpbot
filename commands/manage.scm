            ...
            ((command (list->string (car args)))
             (cmds
                (hashmap
                  ...
                  "unload" (lambda ()
                             (remove-command! (list->string (cadr args)))
                             (irc-reply msg (concat "Unloaded command: " (list->string (cadr args))))
                             )

                  "load" (lambda ()
                             (load-command! (list->string (cadr args)))
                             (irc-reply msg (concat "loaded command: " (list->string (cadr args))))
                             )
                  )))
